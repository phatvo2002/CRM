...
const commonMapDataAutocomplete = (data, label) => {
  ...
  const tempData = data?.map((ele) => {
    ...
    return {
      ...
      label: ele?.name ?? ele?.tenGiaiDoan ?? ele?.tenPhongban ?? ele?.tenBaoGia ??ele?.tenCoHoi ??  ele?.tenNguonGoc?? ele?.tenLoaiTiemNang ?? ele?.tenLoaiHinh ?? ele?.tenKhachHang ?? ele?.tenLinhVuc ?? ele?.tenNganhNghe ?? ele?.tenDoanhThu ?? ele?.tenCuocGoi ?? `${ele?.hoVaDem} ${ele?.ten}` ,
    };
  });
