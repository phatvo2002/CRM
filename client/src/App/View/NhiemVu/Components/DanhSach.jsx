...
import { useDeleteNhiemVuMutation, useGetNhiemVuByPhongBanIdQuery } from 'src/App/Api/NhiemVuApi';
    ...
    const columnsNhiemVu = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                ...
                return <Chip icon={<AssignmentIcon color='white' />} sx={style} label={status} variant="filled" />;
            }
        },
        {
            field: "mucDoUuTien",
                    ...
                    backgroundColor:
                        status == "Cao"
                            ...
                            : status === "Trung bình"
                                ...
                                : status === "Thấp"
                                    ? "#357a38"
                                    : status === "Không xác định" ? "black" : "gray",
                    borderRadius: "30px",
                    fontStyle: "bold"
                };
                return (
                    <div>
                        {status === "Cao" ? <div>
                            <Chip icon={<ShowChartIcon color='white' />} sx={style} label={params?.row?.mucDoUuTien?.name} variant="filled" />
                        </div> : <div>
                            <Chip icon={<LinearScaleIcon color='white' />} sx={style} label={params?.row?.mucDoUuTien?.name} variant="filled" />
                        </div>}
        ...
        { field: "moTa", headerName: "Mô tả", width: 200 },
        {
            ...
            width: 200,
            renderCell: (params) => (
                <div style={{ alignItems: "center" }}><Moment format="DD/MM/YYYY HH:SS">{new Date(params?.row?.hanHoanThanh)}</Moment></div>
            ),
        },
