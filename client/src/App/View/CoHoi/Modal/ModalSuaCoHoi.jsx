...
import { useAddHangHoaQuanTamMutation, useDeleteHangHoaQuanTamMutation, useGetHangHoaQuanTamByKhachHangIdQuery, useUpdateHangHoaQuanTamMutation } from 'src/App/Api/HangHoaQuanTam';
...
import { useGetAllLoaiCoHoiQuery, useGetAllNguonGocKhachHangQuery } from 'src/App/Api/GetDataApi';
...
const ModalSuaCoHoi = ({ showModal,
    ...
    const _isMounted = useRef(false),
        ...
        [khachHangId, setKhachHangId] = useState("")
    const [hangHoa, setHangHoa] = useState([]);
    const [selectedGiaiDoan, setSelectedGiaiDoan] = useState(null);
    const { data: rows, refetch : refetchHangHoaQuanTam} = useGetHangHoaQuanTamByKhachHangIdQuery(khachHangId);
    ...
    const { data: dataLienHe, isLoading: isGetLienHeIsFetching } =
        useGetLienHeByKhachHangMucTieuIdQuery(khachHangId, { skip: showModal == false });
    ...
    const submitForm = (data) => {
        ...
        callApiConvert = async (paramData) => {
            try {
                ...
                toast.success("Thêm mới cơ hội thành công")
        ...
        modalRef.current?.reset(
          {
            ...selectedItem,
            id: selectedItem?.id,
            [modelObj.maKhachHang]: selectedItem[modelObj.maKhachHang],
            [modelObj.tenCoHoi]: selectedItem[modelObj.tenCoHoi],
            [modelObj.soTien]: selectedItem[modelObj.soTien],
            [modelObj.tiLeThanhCong]: selectedItem[modelObj.tiLeThanhCong],
            [modelObj.doanhSoKyVong]: selectedItem[modelObj.doanhSoKyVong],
            [modelObj.ngayKyVongKetThuc]: selectedItem[modelObj.ngayKyVongKetThuc],
            [modelObj.maLienHe]: selectedItem[modelObj.maLienHe],
            [modelObj.maLoaiHangHoa]: selectedItem[modelObj.maLoaiHangHoa],
            [modelObj.maLoaiCoHoi]: selectedItem[modelObj.maLoaiCoHoi],
            [modelObj.maGiaiDoanBanHang]: selectedItem[modelObj.maGiaiDoanBanHang],
