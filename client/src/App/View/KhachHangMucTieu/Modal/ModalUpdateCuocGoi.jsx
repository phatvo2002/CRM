...
const ModalUpdateCuocGoi = (props) => {
  const {
      showModal,
      closeModal,
      typeModal,
    ...
    { id } = useParams(),
    [updateCuocGoi ,{isLoading: isUpdateCuocGoi} ] = useUpdateCuocGoiMutation(),
    header = "Cập nhật cuộc gọi";
    const {data : loaiCuocGoiData , isLoading :isGetCuocGoiFetching} = useGetAllLoaiCuocGoiQuery()
    const {data : KetQuaCuocGoiData , isLoading :isGetKetQuaCuocGoiFetching} = useGetAllKetQuaCuocGoiQuery()
    const isLoading = isUpdateCuocGoi ||isGetCuocGoiFetching||isGetKetQuaCuocGoiFetching
...
export default ModalUpdateCuocGoi;
