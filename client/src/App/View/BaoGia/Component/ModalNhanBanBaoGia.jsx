...
export const ModalNhanBanBaoGia = ({
selectedRow,
  showModal,
  closeModal,
  typeModal,
  ...
  const { data: rows } = useGetHangHoaQuanTamByBaoGiaIdQuery(selectedRow[0]?.id , {skip : showModal == false}); 
  ...
  const submitForm = (data) => {
    ...
    getInitialStateFromApiToUpdate = async (selectedRow) => {
      modalRef.current?.reset(
        {
          ...selectedRow,
          [modelObj.tenBaoGia]: selectedRow?.tenBaoGia,
          [modelObj.ngayBaoGia]: selectedRow?.ngayBaoGia,
          [modelObj.ngayHetHan]: selectedRow?.ngayHetHan,
          [modelObj.diaChi]: selectedRow?.diaChi,
          [modelObj.moTa]: selectedRow?.moTa,
          [modelObj.maSoThue]: selectedRow?.maSoThue,
        //   [modelObj.tongTien]: selectedRow[0]?.id,
          [modelObj.maTinhTrangBaoGia]: selectedRow?.maTinhTrangBaoGia,
          [modelObj.maCoHoi]: selectedRow?.maCoHoi,
          [modelObj.maKhachHang]: selectedRow?.maKhachHang,
        },
        { keepDirty: true }
      );
    };
  ...
  useEffect(() => {
    if (selectedRow[0]) {
      getInitialStateFromApiToUpdate(selectedRow[0]);
    }
  }, [selectedRow[0]]);
  useEffect(() => {
